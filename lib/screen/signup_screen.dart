...
// ── Layout constants ──────────────────────────────────────────────────────────
const double _cardW = 640;
const double _cardH = 820;
...
const double _panelPadH = 76; // left panel horizontal padding
const double _panelPadV = 80; // left panel vertical padding
const double _hTargetPad = 236; // right panel spacer
const double _vTargetPad = 130; // right panel spacer
// ─────────────────────────────────────────────────────────────────────────────
...
class _SignUpScreenState extends ConsumerState<SignUpScreen>
  ...
  static const _departments = ['CSE', 'AI&DS', 'MECH', 'ECE', 'MBA'];
  static const _roles = ['Staff', 'Admin'];
  ...
  Future<void> _submit() async {
    ...
    final err = await ref
        ...
        .authenticate(_emailCtrl.text, 'admin123');
    if (!mounted) return;
    ...
    if (err == null) {
      ...
      setState(
        () => _error =
            'Registration noted! Use admin@attendance.com / admin123 to login.',
      );
    }
  }
        ...
        children: [
          Row(
            ...
            children: [
              ...
              // RIGHT PANEL — 58 %
              //  panel  ≈ 1113 px
              //  card   =  640 px  → h-pad ≈ 236 px ✓
              //  card h ≈  800 px  → v-pad ≈ 140 px ✓
              // ══════════════════════════════════════════════════════════
              Expanded(
                flex: 58,
                child: Container(
  ...
  Widget build(BuildContext context) {
    ...
    return TextFormField(
      ...
      decoration: InputDecoration(
        ...
        errorStyle: _tsError(c: cs.error),
      ),
      validator: validator,
    );
  }
    ...
    return DropdownButtonFormField<String>(
      ...
      items: items
          .map(
            (e) => DropdownMenuItem(
              value: e,
              child: Text(e, style: _tsInput(c: cs.onSurface)),
            ),
          )
          .toList(),
      onChanged: onChanged,
